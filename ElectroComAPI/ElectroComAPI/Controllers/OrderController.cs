...
namespace ElectroComAPI.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        /// <summary>
        /// Place an order with ElectroCom distributor
        /// </summary>
        ...
        public async Task<ActionResult<object>> PlaceOrder([FromBody] PlaceOrderRequest request)
        {
            try
            {
                _logger.LogInformation("Processing order {OrderId} with ElectroCom", request.OrderId);
                ...
                var distributorOrderId = $"EC{DateTime.UtcNow:yyyyMMddHHmmss}{new Random().Next(1000, 9999)}";

                _logger.LogInformation("Order {OrderId} confirmed with ElectroCom, Distributor Order ID: {DistributorOrderId}", 
                ...
                return Ok(new
                {
                    ...
                    EstimatedDeliveryDays = 5, // ElectroCom delivers in 5 days
                    Items = orderItems
                });
            }
            catch (Exception ex)
